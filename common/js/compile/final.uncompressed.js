...
function window_REFRESH(UserSession, Command, OptionalParameter){
	...
	INTERNAL_XMLHTTPOBJ.onreadystatechange = function() {
		...
		if(INTERNAL_XMLHTTPOBJ.readyState == 4 && INTERNAL_XMLHTTPOBJ.status == 200){
			...
			// change width of pseudo-iframe (DIV) ---------------
			//document.getElementById(UpdateWindow).style.width = 400 + 'px';
...
// -----------------------------------------------------------------------------
function AuthenticateUser(LoginMode){
			...
			if(INTERNAL_XMLHTTPOBJ.responseText == 222){
				...
				switch(LoginMode){
					case 'modal':
						...
						parent.parent.location	=	base_url + "/Dashboard";
					break;
...
// select all contents of textarea  --------------------------------------------
...
// Promo Editor - generate code  -----------------------------------------------
	function GenerateCodePromo(){
		//alert('gen code here');
		document.getElementById('promo_code_to_paste').style.visibility	=	'visible';
}
// -----------------------------------------------------------------------------
...
// Promo Status Change ---------------------------------------------------------
	function promo_adClickPreview(PromoID){
		var URL_To_Open = 'https://friendsnflings.com/Signup-' + PromoID;
...
// Promo Status Change ---------------------------------------------------------
	function promo_status(PromoID, Command){



		var _totalParameters	= 1;
    ...
    function confirmation() {
    	...
    	if (answer){
    		...
    		EditPromoSettings(PromoID, 'edit.promo.status', Command);
    		document.getElementById('promo_delete-' + PromoID).innerHTML 		= '<font color=red><i>DELETED</i></font>';
    	}
    }
		...
		switch(Command){
			case('disable'):
				// submit to ajax handler ----------------------
				EditPromoSettings(PromoID, 'edit.promo.status', Command);
				document.getElementById('promo_disable-' + PromoID).innerHTML 		= '<font color=orange><i>DISABLED</i></font>';
				alert('*** WARNING *** \n Performance will NOT be Recorded');			
			break;
			...
			case('enable'):
				...
				document.getElementById('promo_disable-' + PromoID).innerHTML 		= '<font color=orange><i>ENABLED</i></font>';
			...
			case('updateLander'):
				PromoIDR		=	PromoID.split(":");
				PromoID			= PromoIDR[1];
				LanderID		= PromoIDR[0];
				// submit to ajax handler ----------------------
				EditPromoSettings(PromoID, 'edit.promo.updateLander', LanderID);
				//document.getElementById('promo_disable-' + PromoID).innerHTML 		= '<font color=orange><i>DISABLED</i></font>';
			break;
			...
			case('smartPromo'):
				PromoIDR		=	PromoID.split(":");
				PromoID			= PromoIDR[1];
				Parameter		= PromoIDR[0];
				
				// submit to ajax handler ----------------------
				EditPromoSettings(PromoID, 'edit.promo.updatesmartPromo', Parameter);
				//document.getElementById('promo_disable-' + PromoID).innerHTML 		= '<font color=orange><i>DISABLED</i></font>';
			break;
...
// Promo banner Preview --------------------------------------------------------
	function promo_preview(FileURL, Command, DivID, DivWidth, DivHeight){

    document.getElementById(DivID).style.width 			= DivWidth + 'px';
    document.getElementById(DivID).style.height			= DivHeight + 'px';;
    document.getElementById(DivID).style.background = "#ff0000";
    document.getElementById(DivID).style.top 				= 445 + 'px';
    document.getElementById(DivID).style.right			= 60 + 'px';
    document.getElementById(DivID).style.zIndex			= '900';

	switch(Command){
		case('open'):
			document.getElementById(DivID).style.visibility 				=	'visible';
			document.getElementById(DivID).innerHTML 								= '<img src="'+ FileURL +'">';
		break;
...
// Promo Status Change ---------------------------------------------------------
	function promo_ip_filter(PromoID, Command){

		
		if(Command == 'filter'){
			Parameter	=	 'filter' + ':' + document.getElementById('promo_ip_to_filter').value;
			// submit to ajax handler ----------------------
			EditPromoSettings(PromoID, 'edit.promo.filter', Parameter);
			
			Command			=	Command.split(":");
			Parameter		= Command[1];
			alert(document.getElementById('promo_ip_to_filter').value + ' Now Filtered');

		}else{
			// submit to ajax handler ----------------------
			EditPromoSettings(PromoID, 'edit.promo.filter', Command);
			Command			=	Command.split(":");
			Parameter		= Command[1];
			alert(Parameter + ' Removed from Filter');
		}
...
// Submit on Enter ===================================================
function submitToFunctionOnEnterKey(evt, FunctionName, Parameter_A, Parameter_B){
	Evt 					= (evt) ? evt : event;
	...
	if(charCode == 13){
		switch(FunctionName){
			case('updatePromoHighLevel'):
				updatePromoHighLevel(Parameter_A, Parameter_B);
				alert('Host Site / Name Saved');
				return false;			
			break;
		}
	}
...
// Promo Editor - iFrame  ------------------------------------------------------
	function updatePromoHighLevel(PromoID, Command){

		switch(Command){

				case('promo_name_host'):
					elementToEdit_name	=	document.getElementById('promo_name_host-' + PromoID).value

					// submit to ajax handler ----------------------
					EditPromoSettings(PromoID, 'edit.promo.name', elementToEdit_name);
				break;
	}
...
// Promo Editor - iFrame  ------------------------------------------------------
	function updateIframe(PromoID, Command){


		
		document.getElementById('pBuilder_alert').innerHTML 		= '';
		var _totalThumbs 			= null;
		var _totalParameters	= null;
		var _finalCode				= null;
		...
		switch(Command){
				...
				case('change_Color_red'):
					elementToEdit_color	=	document.getElementById('promo_color_elementToEdit').value
					_totalParameters = 'ff0000:' + elementToEdit_color;

					// submit to ajax handler ----------------------
					EditPromoSettings(PromoID, 'edit.promo.color', _totalParameters);
				break;

				case('change_Color_green'):
					elementToEdit_color	=	document.getElementById('promo_color_elementToEdit').value
					_totalParameters = '00ff00:' + elementToEdit_color;
				...
				case('change_Color_blue'):
					elementToEdit_color	=	document.getElementById('promo_color_elementToEdit').value
					_totalParameters = '0000ff:' + elementToEdit_color;
				...
				case('change_Color_yellow'):
					elementToEdit_color	=	document.getElementById('promo_color_elementToEdit').value
					_totalParameters = 'ffff00:' + elementToEdit_color;

					// submit to ajax handler ----------------------
					EditPromoSettings(PromoID, 'edit.promo.color', _totalParameters);
				break;

				case('change_Color_black'):
					elementToEdit_color	=	document.getElementById('promo_color_elementToEdit').value
					_totalParameters = '000000:' + elementToEdit_color;
				...
				case('change_Color_white'):
					elementToEdit_color	=	document.getElementById('promo_color_elementToEdit').value
					_totalParameters = 'ffffff:' + elementToEdit_color;
				...
				case('change_Color_custom'):
					var customColor;
					
					elementToEdit_color	=	document.getElementById('promo_color_elementToEdit').value
					customColor					=	document.getElementById('promo_custom_color_value').value
					_totalParameters = customColor + ':' + elementToEdit_color;
				...
				// add with to geo promo -------------------------------------------------------------------
				case('add_width'):
					var _current_Width 																											= document.getElementById('myframea').style.width;
					var _current_Height																											= document.getElementById('myframea').style.height;
					var	_new_Width																													= parseInt(_current_Width) + 25 + 'px';

					document.getElementById('promo_code_to_paste').style.visibility 				=	'hidden';

					// set proper image title ------------------------
					if(parseInt(_new_Width) > 325){
						
						// submit to ajax handler ----------------------
						EditPromoSettings(PromoID, 'edit.promo.titleimage', '319');						
					}else if(parseInt(_new_Width) < 325 && parseInt(_new_Width) > 225){

						// submit to ajax handler ----------------------
						EditPromoSettings(PromoID, 'edit.promo.titleimage', '225');
					}else if(parseInt(_new_Width) < 225){

						// submit to ajax handler ----------------------
						EditPromoSettings(PromoID, 'edit.promo.titleimage', '125');						
					}
					// -----------------------------------------------

					if(parseInt(_new_Width) < 945){
						document.getElementById('myframea').style.width					= _new_Width;
						document.getElementById('pBuilder_cWidth').innerHTML 		= '<< ' + _new_Width + ' >>';;

        		if(parseInt(_new_Width) >= 110 && parseInt(_new_Width) <= 225){
        			_totalThumbs = document.getElementById('pBuilder_alert').innerHTML 		= '1 Profile';
        		}else if(parseInt(_new_Width) > 225 && parseInt(_new_Width) < 370){
        			_totalThumbs = document.getElementById('pBuilder_alert').innerHTML 		= '2 Profiles';
        		}else if(parseInt(_new_Width) > 345 && parseInt(_new_Width) < 495){
        			_totalThumbs = document.getElementById('pBuilder_alert').innerHTML 		= '3 Profiles';
        		}else if(parseInt(_new_Width) > 470 && parseInt(_new_Width) < 620){
        			_totalThumbs = document.getElementById('pBuilder_alert').innerHTML 		= '4 Profiles';
        		}else if(parseInt(_new_Width) >= 620 && parseInt(_new_Width) < 720){
        			_totalThumbs = document.getElementById('pBuilder_alert').innerHTML 		= '5 Profiles';
        		}else if(parseInt(_new_Width) >= 720 && parseInt(_new_Width) < 845){
        			_totalThumbs = document.getElementById('pBuilder_alert').innerHTML 		= '6 Profiles';		
        		}else if(parseInt(_new_Width) >= 845){
        			_totalThumbs = document.getElementById('pBuilder_alert').innerHTML 		= '7 Profiles';
        		}

						// compile totals ------------------------------
						_totalParameters	=	_totalThumbs + ':' + _new_Width + ':' + _current_Height;
						
						// submit to ajax handler ----------------------
						EditPromoSettings(PromoID, 'edit.promo.thumbs', _totalParameters);
						
						// update code textarea window -----------------
						document.getElementById('promo_code_to_paste').value = '<iframe src="http://egenhq.com/promo-' + PromoID + '" width= ' + _new_Width + ' height= ' + _current_Height + ' scrolling=no marginwidth=0 marginheight=0 frameborder=0 allowTransparency=true></iframe>';

					}else{
						document.getElementById('pBuilder_alert').innerHTML 		= 'Maximum Width Reached';
					}
				break;


				// subtract with to geo promo --------------------------------------------------------------
				case('subtract_width'):
					var _current_Width 																											= document.getElementById('myframea').style.width;
					var _current_Height																											= document.getElementById('myframea').style.height;
					var	_new_Width																													= parseInt(_current_Width) - 25 + 'px';
        		...
        		if(parseInt(_new_Width) >= 110 && parseInt(_new_Width) <= 225){
        			...
        			_totalThumbs = document.getElementById('pBuilder_alert').innerHTML 		= '7 Profiles';
						...
						document.getElementById('pBuilder_alert').innerHTML 		= 'Minimum Width Reached';
					}
				break;
				...
				case('add_height'):
					var _current_Width 																											= document.getElementById('myframea').style.width;
					var _current_Height 																										= document.getElementById('myframea').style.height;
					var	_new_Height																													= parseInt(_current_Height) + 160 + 'px';

					document.getElementById('promo_code_to_paste').style.visibility 				=	'hidden';

					if(parseInt(_new_Height) < 850){
						document.getElementById('myframea').style.height					= _new_Height;
						//document.getElementById('pBuilder_cWidth').innerHTML 		= '<< ' + _new_Width + ' >>';;
						//_totalThumbs = 14;
						//_addRow = '+ 1';
						EditPromoSettings(PromoID, 'edit.promo.addrow', 'null');

						// update code textarea window -----------------
						document.getElementById('promo_code_to_paste').value = '<iframe src="http://egenhq.com/promo-' + PromoID + '" width= ' + _current_Width + ' height= ' + _new_Height + ' scrolling=no marginwidth=0 marginheight=0 frameborder=0 allowTransparency=true></iframe>';
						
					}else{
						document.getElementById('pBuilder_alert').innerHTML 		= 'Maximum Height Reached';
					}
				break;

				case('subtract_height'):
					var _current_Width 																											= document.getElementById('myframea').style.width;
					var _current_Height 																										= document.getElementById('myframea').style.height;
					var	_new_Height																													= parseInt(_current_Height) - 160 + 'px';
					...
					if(parseInt(_current_Height) > 210){
						document.getElementById('myframea').style.height					= _new_Height;
						//document.getElementById('pBuilder_cWidth').innerHTML 		= '<< ' + _new_Width + ' >>';;
						//_totalThumbs = 7;
						EditPromoSettings(PromoID, 'edit.promo.subtractrow', 'null');

						// update code textarea window -----------------
						document.getElementById('promo_code_to_paste').value = '<iframe src="http://egenhq.com/promo-' + PromoID + '" width= ' + _current_Width + ' height= ' + _new_Height + ' scrolling=no marginwidth=0 marginheight=0 frameborder=0 allowTransparency=true></iframe>';
						
					}else{
						document.getElementById('pBuilder_alert').innerHTML 		= 'Minimum Height Reached';
					}
				break;
...
//CODE NOTES --------------
		...
		//var _IframeID =	document.getElementById(IframeID).value;
		//document.getElementById('myframea').innerHTML = 'bang';
		//document.all.myframea.src=val;
		//document.all.myframea.style.visibility="hidden";
		//document.getElementById('myframea').style.display 					= 'none';
		//document.getElementById('myframea').src 											= 'http://egenhq.com/promo-fefe894954607846999902b486e73652f2174b6dfbe92506765cf1d7ef7e75e4';

		
		//alert(document.getElementById('myframea').style.width);


/*


xThumbs X 120 + 50 = Width
4 = 530
5 = 650
6 = 770
7 = 890
...
4 thumbs require 520px+ width
5 thumbs require 620px+ width
6 thumbs require 720px+ width
7 thumbs require 845px+ width
...
// edit promo settings  --------------------------------------------------------
function EditPromoSettings(PromoID, Command, Parameter){


	var INTERNAL_XMLHTTPOBJ = httpOBJCreator();

	if(!PromoID || !Command || !Parameter){
		alert('Error! Wrong Key or Spoof Attempt - Refresh Page if Error Persists');
	}else{

  	var target_url 		= 'execute';
  	var params 				= '_function=ajax.promo.edit' + '&_PromoID=' + PromoID + '&_Command=' + Command + '&_Parameter=' + Parameter; 
//alert(params);
  	...
  	INTERNAL_XMLHTTPOBJ.onreadystatechange = function(){
		if(INTERNAL_XMLHTTPOBJ.readyState == 4 && INTERNAL_XMLHTTPOBJ.status == 200){
			...
			//alert(INTERNAL_XMLHTTPOBJ.responseText);			//debug

			if(INTERNAL_XMLHTTPOBJ.responseText == 5){
				alert('Promo Deleted');
			}
			
			if(INTERNAL_XMLHTTPOBJ.responseText == 6){
				alert('Updated Promo Landing Page');
			}

			if(INTERNAL_XMLHTTPOBJ.responseText == 71){
				alert('** smartPromo ACTIVATED ** \n\nsmartPromo attempts to find Promo Ads that connect with your visitors, rather than showing the same Promo Ad you originally selected. \n\nsmartPromos will automatically use Promo Ads of the same Size and of equal of lesser Content Rating (e.g. if PG rated, only PG or G will be shown) \n\nThe number of different Promo Ads shown is limited to the number of Matching Promo Ads per the Original Promo Ad you Originally Selected.  As the Promo Ad Library grows, the smartPromo System will become more effective.');
			}

			if(INTERNAL_XMLHTTPOBJ.responseText == 70){
				alert('** smartPromo DEACTIVATED **');
			}
			...
			if(Parameter == 'filter' || Parameter == 'remove' || Parameter == 'delete' || Parameter == 'disable' || Parameter == 'enable'){
				//alert(8);
				//window.location.href = window.location.pathname;
				window.location	=	'MyPromos';
			
			}else if(Command == 'edit.promo.updateLander' || Parameter == 'on' || Parameter == 'off'){
			
			}else{
				document.getElementById('myframea').src 											= 'http://egenhq.com/promo-' + PromoID;
			}
...
// profile spec sheet --------------------------------------------------------------------
function SaveMyProfileBasicsAddress(){
	
	var ProfileBasics_tax_id_ssn=	document.getElementById('ProfileBasics.tax_id_ssn').value;
	var ProfileBasics_tax_payee	=	document.getElementById('ProfileBasics.tax_payee').value;
	var ProfileBasics_firstname	=	document.getElementById('ProfileBasics.first_name').value;
	var ProfileBasics_lastname	=	document.getElementById('ProfileBasics.last_name').value;
	var ProfileBasics_address1	=	document.getElementById('ProfileBasics.address_1').value;
	var ProfileBasics_address2	=	document.getElementById('ProfileBasics.address_2').value;
	var ProfileBasics_city			=	document.getElementById('ProfileBasics.city').value;
	var ProfileBasics_state			=	document.getElementById('ProfileBasics.state').value;
	var ProfileBasics_zipcode		=	document.getElementById('ProfileBasics.zipcode').value;
	var ProfileBasics_phone			=	document.getElementById('ProfileBasics.phone').value;
	var ProfileBasics_Token			=	document.getElementById('ProfileBasics.saveToken').value;
	var ProfileBasics_UIDP			=	document.getElementById('_userid_private').value;



	var target_url				= 'execute';
	var params 						= '_function=ajax.ProfileBasics.Update&ProfileBasics_tax_id_ssn=' + ProfileBasics_tax_id_ssn + '&ProfileBasics_tax_payee=' + ProfileBasics_tax_payee + '&ProfileBasics_lastname=' + ProfileBasics_lastname + '&ProfileBasics_firstname=' + ProfileBasics_firstname + '&ProfileBasics_address1=' + ProfileBasics_address1 + '&ProfileBasics_address2=' + ProfileBasics_address2 + '&ProfileBasics_city=' + ProfileBasics_city + '&ProfileBasics_state=' + ProfileBasics_state + '&ProfileBasics_zipcode=' + ProfileBasics_zipcode + '&ProfileBasics_Token=' + ProfileBasics_Token + '&ProfileBasics_UIDP=' + ProfileBasics_UIDP + '&ProfileBasics_phone=' + ProfileBasics_phone;
	...
	if(INTERNAL_XMLHTTPOBJ.readyState == 4 && INTERNAL_XMLHTTPOBJ.status == 200){
		...
		if(INTERNAL_XMLHTTPOBJ.responseText == 666){
			...
			alert('Information Stored & Sensitive Data Encrypted');
			document.getElementById('ProfileBasics.name').style.backgroundColor	='#d1ffc5';
			document.getElementById('ProfileBasics.address1').style.backgroundColor	='#d1ffc5';
			document.getElementById('ProfileBasics.address2').style.backgroundColor	='#d1ffc5';
			document.getElementById('ProfileBasics.city').style.backgroundColor	='#d1ffc5';
			document.getElementById('ProfileBasics.state').style.backgroundColor	='#d1ffc5';
			document.getElementById('ProfileBasics.zipcode').style.backgroundColor	='#d1ffc5';
		}
...
// -------------------------------------
function onEnter(evt,frm, formMode) {
	var keyCode = null;
	if(evt.which){
		keyCode = evt.which;
	}else if(evt.keyCode){
...
function SignUp_Goto(){
		window.location = window.location.protocol + '//' + window.location.hostname + '/Signup';
}
// -------------------------------------
...
function Goto_Page(target){
		window.location = window.location.protocol + '//' + window.location.hostname + '/' + target;
}
// -------------------------------------
