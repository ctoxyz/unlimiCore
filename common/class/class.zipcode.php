...
class zipcode_class {
   ...
   function get_zips_in_range($zip, $range, $sort=1, $include_base) {
	...
	// ---------------------------------------------------------------------------
   function get_zips_in_range_fast($zip, $range, $sort=1, $include_base) {
      ...
      $sql .= "lat BETWEEN '$min_lat' AND '$max_lat' 
               AND lon BETWEEN '$min_lon' AND '$max_lon' ORDER BY RAND()";
         ...
         return false;
          ...
          do{
            ...
            if ($dist <= $range) {
          	...
          	$__counter++;
          }while(($row = mysql_fetch_row($r)) && $__counter < 6000);
          
          
/*
          PREVIOUS CODE - REMOVED TO MAKE FASTER
